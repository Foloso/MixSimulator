...
class Optimizer:
    ...
    def get_params(self) -> dict:
        return {
            "optimizer(s)": self.__optimizers,
            "budget(s)": self.get_budget(),
            "parametrization": self.get_parametrization(),
            "num_worker": self.get_num_worker(),
        }
    ...
    def optimize(self, mix=None, func_to_optimize=None, constraints=None, step: int = 1, init: int = 0):
        ...
        optimizer = chaining_algo(
            parametrization=self.get_parametrization(), budget=budgets[-1], num_workers=self.get_num_worker()
        )
        ...
        for tmp_budget in tqdm.tqdm(range(0, total_budget)):
            ...
            if (tmp_budget + 1) % step == 0:
                ...
                result_per_budget.update(
                    {"loss": func_to_optimize(recommendation.value, constraints["time_interval"])}
                )
                ...
                if mix is not None:
                    ...
                    try:
                        weighted_coef = mix.get_weighted_coef(
                            usage_coef, time_interval=constraints["time_interval"], init=init
                        )
                    except:
                    ...
                    for t in range(0, len(weighted_coef)):
                        ...
                        try:
                            u_demand += np.abs(
                                mix.get_unsatisfied_demand_at_t(
                                    weighted_coef[t], t, constraints["time_interval"], init=init
                                )
                            )
                        except:
                            u_demand += np.abs(
                                mix.get_unsatisfied_demand_at_t(weighted_coef[t], t, constraints["time_interval"])
                            )
